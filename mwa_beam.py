...
c=constants.c
PI=constants.pi
def mwa_beam(n,freqs):
    l1=np.arange(-1,1,2./n)
    m1=np.arange(-1,1,2./n)
    r=np.sqrt(l1**2+m1**2)
    theta1=np.arccos(l1/r)
    l=r*np.cos(theta1)
    m=r*np.sin(theta1)
    theta=np.arccos(m)
    phi=np.arcsin(l)
    ...
    freqs=freqs
    beampwr=np.empty(l.size*m.size*freqs.size).reshape(l.size,m.size,freqs.size)
