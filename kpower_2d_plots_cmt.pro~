...
;;     power, noise_meas , weights, noise_expval, kperp_edges, kpar_edges, kperp_bin, kpar_bin, kperp_lambda_conv, delay_params, hubble_param
...
;;   noise_meas : a 2d measured noise array in same space as power
...
pro kpower_2d_plots_cmt, power_savefile, power = power, noise_meas = noise_meas, weights = weights, noise_expval = noise_expval, $
  ...
  if keyword_set(plot_noise) then begin
    if n_elements(noise_meas) eq 0 then message, 'noise is undefined in this file'
    power = noise_meas
    plot_type = 'noise'
  endif
  
  if keyword_set(nnr) then begin
    power = noise_meas / noise_expval
    wh_err0 = where(noise_expval eq 0, count_err0)
    if count_err0 gt 0 then power[wh_err0] = 0
...
;print,kpar_log_diffs,total(abs(kpar_log_diffs - kpar_log_diffs[0])),n_kpar
